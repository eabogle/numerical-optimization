...
%xiters = vector from [0:k] so the correct iteration is given for the
%         funvect and norgradvect who start with iteration 0 in slot 1 of
%         the arrays defined
xiters = 0:1:k;

%normplot
figure;
plot(xiters, log10(normgradvect), '-*b')
...
[X,Y]=meshgrid(-1.5:0.1:2, -2:0.1:4);
Z=(1-X).^2 + A.*(Y-X.^2).^2;
